...
Usage (from ai-module/, with the project venv active):
    .venv/Scripts/python.exe split_brand_val.py          # preview only
    .venv/Scripts/python.exe split_brand_val.py --apply  # actually move files
"""
...
def main():
    ...
    for class_dir in sorted(train_root.iterdir()):
        ...
        print(
            f"{class_dir.name}: {len(images)} images total "
            f"(own={len(own_paths)}, compcars={len(compcars_paths)}) -- "
